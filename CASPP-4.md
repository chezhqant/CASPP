...
7.  Y86-64的一条指令可以分为以下阶段：      
    ...
    + 访存（memory）。访存阶段可以将数据写入内存，或者从内存读出数据。读出的值位valM。      
...
8.  Y86-64指令 OPQ/rrmoveq和irmovq在顺序实现中的计算。这些指令计算了一个值，并将结果存放在寄存器中。符号iconde:ifun表明指令字节的两个组成部分，而rA:rB表明寄存器指示符字节的两个组成部分。符号M<sub>1</sub>[x]表示访问内存位置x处的一个字节，而M<sub>8</sub>[x]表示访问8个字节:          
    ...
    访存阶段参考书中的rmmovq和mrmovq过程        
    pushq和popq 参考书中      
    jxx, call, ret参考书中      
9.  标识顺序实现        
    |阶段|计算|
    |-|-|
    |取值|icode:ifun<br>rA,rB<br>valC<br>valP|
    |译码|valA, srcA<br>valB, srcB|
    |执行|valE<br>Cond.codes|
    |访存|read/write|
    |写回|E port, dstE<br>M port, dstM|
    |更新PC|PC|

10. 六个阶段的硬件实现参考书上      
11. 流水线话的一个重要特性就是提高了系统的屯储量。但是会轻微的增加延迟。      
12. 吞吐量，延迟    
    + 吞吐量=指令/执行指令的时间      
    + 从头到尾执行一条指令所需的时间称为延迟      
13. 流水线的局限性，是由于插入了流水线寄存器    
    + 不一致的划分。流水线将一条指令分为N个阶段。但是每个阶段的延迟是不一样的。但是每个阶段的延迟时间是按照最大的计算的。     
    + 流水线过深，收益反而下降        
14. 带反馈的流水线系统，相关会导致冒险    
    + 数据相关会导致数据冒险      
    + 指令相关会导致控制冒险      
