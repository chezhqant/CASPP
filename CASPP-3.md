...
36. 当调用一个过程的时候，某些时候需要将数据作为参数传递，有的时候还需要返回一个值。x86-64的大部分过程见的数据传送是通过寄存器实现的。寄存器的使用是有特殊的顺序的。寄存器使用的名字取决于要传递的数据类型的大小。如果一个函数有大于6个整形参数，超出6个的部分就要通过栈来传递。而第7~n号参数的存储空间要在 P 过程的栈中分配。而这些参数在栈中的位置呢，具体看图。所有的参数数据大小都向8对其。
